...
description: >-
  ...
  ongoing orchestration loop for a Jira epic or a standalone project — read the manifest, compute the next unblocked phase, show the rail view, hand each phase to an executor (Codex CLI itself, or a Codex subagent), record progress and per-phase notes, and absorb runtime roadmap changes. Use whenever a roadmap already exists and work needs to advance, resume, or be re-checked. The roadmap is the single source of truth — always re-read it.
...
3. Dispatch the phase to the **executor** — Codex CLI itself, or a Codex subagent (e.g. via the `agent` / `task` flow) — passing the phase `scope` and `done` criteria.
